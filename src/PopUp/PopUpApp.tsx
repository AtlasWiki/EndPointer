...
function PopUpApp() {
  ...
  function parseURLs(){
    browser.tabs.query({ active: true, currentWindow: true }).then((tabs) => {
      if (tabs[0]?.id) {
        browser.tabs.sendMessage(tabs[0].id, { action: 'parseURLs' }).then((response) => {
  ...
  useEffect(() => {
    browser.storage.local.get(['urlParser', 'jsFileCounter', 'jsFileCount']).then((result) => {
      setURLParser(result.urlParser as boolean || false)
    });
  }, []);
 
  function updateURLCount() {
    browser.storage.local.get("URL-PARSER").then((data) => {
        const urlParser = data["URL-PARSER"] as Record<string, any> | undefined;
  ...
  function urlParserState() {
    ...
    browser.runtime.sendMessage({ action: 'urlParserChanged', state: newState })
  }
  ...
  useEffect(() => {
    browser.storage.local.get("scope").then((result) => {
      setScopes(result.scope as string[] || [])
    });
  }, []);
  ...
  const handleRemoveScope = (scopeToRemove: string) => {
    setScopes((prevScopes) => {
      const updatedScopes = prevScopes.filter(scope => scope !== scopeToRemove);
      browser.storage.local.set({ scope: updatedScopes });
      return updatedScopes;
    });
  };
  ...
  useEffect(() => {
    browser.storage.local.get("requests").then((result) => {
      setReqAmt(result.requests as number || 1)
    });
  }, []);
  ...
  return (
    <div className="w-full md:h-screen m-0  md:justify-center py-5"> 
      ...
      <div className="w-full text-center flex flex-col justify-center items-center">
        ...
        <button className='mt-2' onClick={() => {browser.storage.local.set({scope: []}); setScopes([])}}>
