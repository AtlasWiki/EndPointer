(function () {
	...
	const REL_REGEX = /["']((\/|(\w+\/))[\w./\-_?=&]*)["']/g;
	const ABS_REGEX = /(https?:\/\/[^\s'"){}(]+)/g;

	let parsedJSFiles = new Set();
	let successfullyFetchedFiles = new Set();
	let failedFetchAttempts = new Map();
	...
	async function processJSFile(js_file) {
	...
	async function saveToStorage(encodedURL, urls) {
	    const result = await browser.storage.local.get('URL-PARSER');
	    const urlParser = result['URL-PARSER'] || {};
	...
	async function updateURLCount(count) {
	    await browser.storage.local.set({ urlCount: count });
	}
	async function updateJSFileCount(count) {
  ...
  `;
	...
	function updateProgress(progress, status) {
	    let progressBar = document.getElementById('parsing-progress-container');
	    if (!progressBar) {
